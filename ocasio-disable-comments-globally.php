<?php
/**
 * Plugin Name: Ocasio Disable Comments Globally
 * Plugin URI:  https://kevinocasio.com/wordpress-plugins/ocasio-disable-comments-globally/
 * Description: Completely disables and removes the WordPress comment system across all posts, pages, and media files.
 * Version:     1.0.0
 * Author:      Kevin Ocasio
 * Author URI:  https://kevinocasio.com
 * License:     GPL-2.0-or-later
 * License URI: https://www.gnu.org/licenses/gpl-2.0.html
 * Text Domain: ocasio-disable-comments-globally
 */
...
define('OCASIO_DCG_VERSION', '1.0.0');
define('OCASIO_DCG_PLUGIN_DIR', plugin_dir_path(__FILE__));
define('OCASIO_DCG_PLUGIN_URL', plugin_dir_url(__FILE__));

/**
 * Helper: Smart Brand URL Resolver (Keeps links local on .local, points to live domain on production)
 */
function ocasio_dcg_brand_url($path = '/wordpress-plugins/') {
 ...
 */
function ocasio_dcg_activate() {
	if (get_option('ocasio_dcg_enabled') === false) {
		$legacy = get_option('ko_dcg_enabled', 1);
		update_option('ocasio_dcg_enabled', (int) $legacy);
	}
}
register_activation_hook(__FILE__, 'ocasio_dcg_activate');

/**
 * 2. Core Logic: Disable Comments Site-Wide
 */
function ocasio_dcg_is_active() {
	$enabled = get_option('ocasio_dcg_enabled');
	if ($enabled === false) {
		$enabled = get_option('ko_dcg_enabled', 1);
	}
	return (int) $enabled === 1;
}

// 2.1 Disable post type support for comments and trackbacks
add_action('admin_init', function () {
	if (!ocasio_dcg_is_active()) {
		return;
	}
...
// 2.2 Close comments and pings on the front-end
add_filter('comments_open', function ($open) {
	return ocasio_dcg_is_active() ? false : $open;
}, 20, 2);
...
// 2.3 Hide existing comments
add_filter('comments_array', function ($comments) {
	return ocasio_dcg_is_active() ? array() : $comments;
}, 10, 2);

// 2.4 Remove Comments page from admin menu
...
// 2.5 Redirect direct access to comments screen
add_action('admin_init', function () {
	if (!ocasio_dcg_is_active()) {
		return;
	}
...
// 2.6 Remove Comments from admin bar
 ...
 */
function ocasio_dcg_register_settings() {
	register_setting('ocasio_dcg_settings_group', 'ocasio_dcg_enabled', array(
		'type'              => 'integer',
		'sanitize_callback' => 'absint',
		'default'           => 1,
	));
}
add_action('admin_init', 'ocasio_dcg_register_settings');

/**
 * 4. Register Admin Menu under Ocasio Plugins (Position 65)
 */
function ocasio_dcg_register_menu() {
...
add_action('admin_menu', 'ocasio_dcg_register_menu');

/**
 * 5. Action Links on Plugins Screen
 */
function ocasio_dcg_action_links($links) {
	$settings_link = '<a href="' . esc_url(admin_url('admin.php?page=ocasio-plugins-main')) . '">' . esc_html__('Dashboard', 'ocasio-disable-comments-globally') . '</a>';
	array_unshift($links, $settings_link);
	return $links;
}
add_filter('plugin_action_links_' . plugin_basename(__FILE__), 'ocasio_dcg_action_links');

/**
 * 6. Conditional Admin Asset Enqueuing
 */
function ocasio_dcg_admin_assets($hook) {
	...
	if (strpos($hook, 'ocasio-plugins-main') !== false || $hook === 'toplevel_page_ocasio-plugins-main') {
		wp_enqueue_style('ocasio-dcg-admin-css', plugins_url('assets/admin.css', __FILE__), array(), OCASIO_DCG_VERSION);
		wp_enqueue_script('ocasio-dcg-admin-js', plugins_url('assets/admin.js', __FILE__), array('jquery'), OCASIO_DCG_VERSION, true);
		wp_localize_script('ocasio-dcg-admin-js', 'ocasio_vars', array(
			'ajaxurl'     => admin_url('admin-ajax.php'),
			'suite_nonce' => wp_create_nonce('ocasio_suite_toggle_nonce'),
		));
		wp_localize_script('ocasio-dcg-admin-js', 'ocasio_301_vars', array(
			'ajaxurl'     => admin_url('admin-ajax.php'),
			'suite_nonce' => wp_create_nonce('ocasio_suite_toggle_nonce'),
		));
	}
}
add_action('admin_enqueue_scripts', 'ocasio_dcg_admin_assets');
